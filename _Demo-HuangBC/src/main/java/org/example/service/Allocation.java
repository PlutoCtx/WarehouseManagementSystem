...
public class Allocation {
    ...
    public static void Allocation(int id1,int id2) {
        ...
        if(out1.getBj()!=4||out2.getBj()!=5) {
            Logger.getGlobal().info("不是调拨记录，无法处理");
            return;
        }
        if(out1.getChecked()==0||out2.getChecked()==0) {
            Logger.getGlobal().info("该调拨记录尚未通过审核，无法处理");
            return;
        }
        if(out1.getVs()==1||out2.getVs()==1) {
            Logger.getGlobal().info("该调拨记录已处理过，无需再处理");
            return;
        }
        ...
        if(dRecord1==null||dRecord2==null) {
            Logger.getGlobal().info("仓库为空，无法调拨");
            return;
        }
        ...
        try {
            ...
            Logger.getGlobal().info("WarehousingService.Warehousing()事务处理异常");
            try {
                DBConnect.conn.rollback();
